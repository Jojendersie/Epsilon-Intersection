...
namespace ei {
    ...
    /*static bool isConvexComb(const Vec3& _x, const Vec3& _v0, const Vec3& _v1, const Vec3& _v2, float _threshold, Plane& _outPlane, float& _d)
    {
        ...
        // Point is within slab around the plane. Is it's projection in the
        // triangle?
        /*float det = a.x * b.y - a.y * b.x;
        if(det == 0.0f) return false;
        Vec3 x = _x - _v0;
        float b0 = (x.x * b.y - x.y * b.x) / det;
        float b1 = (a.x * x.y - a.y * x.x) / det;
        return b0 >= 0.0f
        /*float da = dot(a, _x - _v0);
        float db = dot(b, _x - _v0);
        da + db
        float barycentricCoord1 = dot(, b) * /
    }
    ...
    uint32 convexSet(Vec3* _points, uint32 _numPoints, float _threshold)
    {
        ...
        for(uint32 i = 0; i < _numPoints; ++i)
        {
            ...
            for(uint32 j = 0; j < _numPoints && maySeparate; ++j) if(i != j)
            {
                ...
                case 2: {
                    // If j is a convex combination of the other two skip it
                    if(distance(_points[j], Segment(extrema[0], extrema[1])) > _threshold)
                    {
                        // Add it
                        extrema[ne++] = _points[j];
                        // Test if i itself is a convex combination -> no extrema
                        //p = Plane(extrema[0], extrema[1], extrema[2]);
                        //d = dot(_points[i], p.n) + p.d;
                        if(isConvexComb(_points[i], extrema[0], extrema[1], extrema[2], _threshold, p, d))
                            maySeparate = false;
                    }
                } break;
                default: {
                    ...
                    if(dj * d > 0.0f) // Not separated by current plane?
                    {
                      ...
                      //  d = dot(_points[i], p.n) + p.d;
                        if(isConvexComb(_points[i], extrema[j], extrema[1], extrema[2], _threshold, p, d)) { maySeparate = false; break; }
                      ...
                      //  d = dot(_points[i], p.n) + p.d;
                        if(isConvexComb(_points[i], extrema[0], extrema[j], extrema[2], _threshold, p, d)) { maySeparate = false; break; }
                        dj = dot(extrema[1], p.n) + p.d;
                      //  if(abs(d) <= _threshold) { maySeparate = false; break; }
                        if(d * dj < 0.0f) { extrema[1] = _points[j]; break; }
                      //  p = Plane(extrema[0], extrema[1], _points[j]);
                      //  d = dot(_points[i], p.n) + p.d;
                        if(isConvexComb(_points[i], extrema[0], extrema[1], extrema[j], _threshold, p, d)) { maySeparate = false; break; }
