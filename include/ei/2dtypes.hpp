...
namespace ei {
    ...
    // AREA METHODS
    // ************************************************************************* //
    inline float area(const Disc2D& _disc) noexcept                            // TESTED
    {
        return _disc.radius * _disc.radius * PI;
    }

    inline float area(const Rect2D& _rect) noexcept                            // TESTED
    {
    ...
    inline float area(const ORect2D& _orect) noexcept                          // TESTED
    {
        eiAssert(_orect.size >= 0.0f, "Rect must have positive side lengths!");
        return _orect.size.x * _orect.size.y;
    }

    inline float area(const Triangle2D& _triangle) noexcept                    // TESTED
    {
    ...
    inline float area(const Ellipse2D& _ellipse) noexcept                      // TESTED
    {
        return PI * _ellipse.radii.x * _ellipse.radii.y;
    }

    inline float area(const OEllipse2D& _oellipse) noexcept                    // TESTED
    {
        return PI * _oellipse.radii.x * _oellipse.radii.y;
    }

    inline float area(const Segment2D&) noexcept                               // TESTED
    {
        return 0.0f;
    }

    inline float area(const Ray2D&) noexcept                                   // TESTED
    {
        return 0.0f;
    }

    inline float area(const Capsule2D& _capsule) noexcept                      // TESTED
    {
    ...
    // CENTROID METHODS
    // ************************************************************************* //
    inline Vec2 center(const Disc2D& _disc) noexcept                           // TESTED
    {
        return _disc.center;
    }

    inline Vec2 center(const Rect2D& _rect) noexcept                           // TESTED
    {
        return 0.5f * (_rect.min + _rect.max);
    }

    inline Vec2 center(const ORect2D& _orect) noexcept                         // TESTED
    {
        return _orect.center;
    }

    inline Vec2 center(const Triangle2D& _triangle) noexcept                   // TESTED
    {
        return (_triangle.v0 + _triangle.v1 + _triangle.v2) / 3.0f;
    }

    inline Vec2 center(const Ellipse2D& _ellipse) noexcept                     // TESTED
    {
        return _ellipse.center;
    }

    inline Vec2 center(const OEllipse2D& _oellipse) noexcept                   // TESTED
    {
        return _oellipse.center;
    }

    inline Vec2 center(const Segment2D& _segment) noexcept                     // TESTED
    {
        return (_segment.a + _segment.b) * 0.5f;
    }

    inline Vec2 center(const Capsule2D& _capsule) noexcept                     // TESTED
    {
