...
namespace ei {
    ...
    template<typename T>
    EIAPI constexpr inline bool isPrime(T _n)
    {
    ...
    template<>
    EIAPI constexpr inline bool isPrime(uint64 _n)
    {
        // No negative primes and no 0 and 1.
        if(_n < 2) return false;
        if(_n == 2 || _n == 3) return true;
    ...
    template<>
    EIAPI constexpr inline bool isPrime(int64 _n) { return isPrime(static_cast<uint64>(_n)); }
    ...
    template<typename T>
    EIAPI constexpr inline T nextPrimeGreaterOrEqual(T _number)
    {
    ...
    ///    _number otherwise.
    template<typename T>
    EIAPI constexpr inline T nextPrime(T _number)
    {
