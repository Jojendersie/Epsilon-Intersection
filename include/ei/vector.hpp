...
#       define ENABLE_IF(condition) typename = typename std::enable_if< (condition) >::type
        ...
        template<typename T1>
        EIAPI explicit Matrix(T1 _a0) noexcept :
            details::Components<T,M,N>(_a0)
        {}
        template<typename T1, typename T2, typename... Args>
        EIAPI Matrix(T1 _a0, T2 _a1, Args... _args) noexcept :
    ...
    public:
        ...
        TQuaternion( T _x, T _y, T _z ) noexcept // TESTED
        {
            double halfAngle;
        ...
        TQuaternion( const Vec<T,3>& _eulerAngles ) noexcept :
        ...
        TQuaternion( const Matrix<T,3,3>& _matrix ) noexcept : // TESTED
            TQuaternion<T>(transpose(_matrix(0)), transpose(_matrix(1)), transpose(_matrix(2)))
        {}

        /// \brief Create from orthogonal basis vectors.
        TQuaternion( const Vec<T,3>& _xAxis, const Vec<T,3>& _yAxis, const Vec<T,3>& _zAxis ) noexcept 
        {
            // Check handness
        ...
        TQuaternion( T _i, T _j, T _k, T _r ) noexcept :
            i(_i), j(_j), k(_k), r(_r)
        {}
        ...
        TQuaternion( const Vec<T,3>& _from, const Vec<T,3>& _to ) noexcept // TESTED
        {
        ...
        bool operator == (const TQuaternion& _q1) const noexcept
        {
        ...
        TQuaternion& operator *= (const TQuaternion& _q1) noexcept
        {
        ...
        TQuaternion& operator *= (T _s) noexcept
        {
        ...
        TQuaternion operator * (const TQuaternion& _q1) const noexcept
        {
           // TQuaternion q0 = *this;
            return TQuaternion(*this) *= _q1;
        }
        TQuaternion operator * (T _s) const noexcept
        {
            return TQuaternion(*this) *= _s;
        }
        TQuaternion operator / (TQuaternion _q1) const noexcept
        {
            return _q1 /= *this;
        }
        TQuaternion operator / (T _s) const noexcept
        ...
        TQuaternion operator - () const noexcept // TESTED
        {
...
namespace ei {
    ...
    EIAPI inline Mat3x3 rotation( const Vec3& _eulerAngles )         { return rotation(_eulerAngles.x, _eulerAngles.y, _eulerAngles.z); }

    // ********************************************************************* //
    /// \brief Rotation matrix in 3D/homogeneous space for an arbitrary axis.
    inline Mat3x3 rotation( const Vec3& _axis, float _angle ) noexcept
    {
        float sinA = sin(_angle);
        float cosA = cos(_angle);
    ...
    // in debug mode.
    template<typename T>
    EIAPI inline bool decomposeCholesky(const Matrix<T,2,2>& _A, Matrix<T,2,2>& _L) noexcept // TESTED
    {
    ...
    template<typename T>
    EIAPI inline bool decomposeCholesky(const Matrix<T,3,3>& _A, Matrix<T,3,3>& _L) noexcept // TESTED
    {
        if(_A[0] <= 0.0f) return false;
        _L[0] = sqrt(_A[0]);
        _L[1] = 0.0f;
