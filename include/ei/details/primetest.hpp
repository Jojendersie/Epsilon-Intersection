namespace ei { namespace details {
    ...
    template<typename T>
    EIAPI inline bool isPrime_DivisorBasedTest(T _n)
    {
    ...
    EIAPI inline bool millerRabinTest(const uint32 _n, const uint32 _a)
    {
    ...
    EIAPI inline bool isPrime_MRTest(uint32 _n)
    {
        if(_n % 2 == 0 || _n % 3 == 0) return false;
